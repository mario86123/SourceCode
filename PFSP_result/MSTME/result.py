...
for i in range(len(ell_lst)):
    for j in range(len(pop_size_lst)):
            ...
            for t in range(repeat_time):
                ...
                f = open(f'./{ell_lst[i]}_{pop_size_lst[j] * ell_lst[i]}_MSTME_{t}.txt', 'r')
                s = f.read().splitlines()
            ...
            # print result data
            print(f'problem: {ell_lst[i]}, population size: {pop_size_lst[j] * ell_lst[i]}, model = MSTME:')
