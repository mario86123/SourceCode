...
do
  ...
  do
      ...
      do
          echo "problem = tai"${ProblemSizeArray[$ell]}"a.dat.dat, pop_size = "$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))", times = ${times}, Emax = "$((${ProblemSizeArray[$ell]} * ${ProblemSizeArray[$ell]} * 1000))", model = MR"

          ./RankingEDAsCEC -i ./QAP_instance/tai"${ProblemSizeArray[$ell]}"a.dat.dat\
                            -o ./QAP_result/MR/"${ProblemSizeArray[$ell]}"a_"$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))"_MR_"$times".txt\
                            -s "$times"\
                            -t QAP\
                            -m MR -d C -v 0\
