...
repeat_time = 10
ell_lst = [20, 40, 60]
pop_size_lst = [1, 3, 5, 10, 20]
cut_points_count_lst = ["1", "1.5", "2", "3", "4", "5"]
model_lst = ["MRWT", "MRWFT"]
...
for ell in ell_lst:
    for pop_size in pop_size_lst:
        ...
        for cut_points_count in cut_points_count_lst:
            for model in model_lst:
                ...
                for t in range(repeat_time):
                    ...
                    f = open(f'./{ell}_{ell * pop_size}_{model}{cut_points_count}_{t}.txt', 'r')
                    s = f.read().splitlines()
                ...
                # print result data
                print(f'ell: {ell}, population size: {ell * pop_size}, model = {model}, cut points = {cut_points_count}:')
