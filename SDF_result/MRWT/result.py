...
cut_points_count_lst = ["1", "2", "3", "4", "5"]
model_lst = ["MRWT"]
    ...
    for pop_size in pop_size_lst:
        ...
        for cut_points_count in cut_points_count_lst:
            ...
            for t in range(repeat_time):
                ...
                f = open(f'./{ell}_{ell * pop_size}_MRWT{cut_points_count}_{t}.txt', 'r')
                s = f.read().splitlines()
            ...
            # print result data
            print(f'ell: {ell}, population size: {ell * pop_size}, model = MRWT, cut points = {cut_points_count}:')
