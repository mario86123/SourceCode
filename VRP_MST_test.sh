...
do
  ...
  do
      ...
      do
          echo "problem = ${Problem[$ell]}, pop_size = "$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))", times = ${times}, Emax = "$((${ProblemSizeArray[$ell]} * ${ProblemSizeArray[$ell]} * 1000))", model = MST"

          ./RankingEDAsCEC -i ./VRP_instance/"${Problem[$ell]}".vrp\
                            -o ./VRP_result/MST/"${Problem[$ell]}"_"$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))"_MST_"$times".txt\
                            -s "$times"\
                            -t VRP\
                            -m MST -d C -v 0\
