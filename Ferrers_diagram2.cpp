...
void Ferrers_diagram2::random_SYT_greene_niejenhuis_wilf(){
    ...
    do{
        ...
        if (corner_i + 1 == ferrers_shape_length_dynamic_ && corner_j == 0)
            ferrers_shape_length_dynamic_--;
        //delete box
...
void Ferrers_diagram2::walk(int rand_cell, int *cell_i, int* cell_j){
...
long double Ferrers_diagram2::calculate_hook_length_greene_niejenhuis_wilf(){
    Generic gen;
    if( hook_length_ == -1 ){
        hook_length_ = 1 ;
        ...
        for (int i = 0 ; i < ferrers_shape_length_; i ++){
            for (int j = 0 ; j < ferrers_shape_[ i ]; j++) {
                ...
                //if(cell_hook == -1 ){
                //  toStr();
                //get_num_cells_down(i, j);}
            }
        }
        hook_length_ = gen.factorial(partition_of_n_) / hook_length_;
        number_of_permutations_ = hook_length_ * hook_length_;
    }
    return hook_length_;
}

int Ferrers_diagram2::get_num_cells_down(int i, int j){
