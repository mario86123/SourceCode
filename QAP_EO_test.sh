...
do
  ...
  do
      ...
      do
          echo "problem = tai"${ProblemSizeArray[$ell]}"a.dat.dat, pop_size = "$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))", times = ${times}, Emax = "$((${ProblemSizeArray[$ell]} * ${ProblemSizeArray[$ell]} * 1000))", model = EO"

          ./RankingEDAsCEC -i ./QAP_instance/tai"${ProblemSizeArray[$ell]}"a.dat.dat\
                            -o ./QAP_result/EO/"${ProblemSizeArray[$ell]}"a_"$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))"_EO_"$times".txt\
                            -s "$times"\
                            -t QAP\
                            -m EO -d C -v 0\
