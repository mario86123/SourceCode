...
do
  ...
  do
      ...
      do
          ...
          echo "problem = tai"${Problem[$ell]}".txt, pop_size = "$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))", times = ${times}, Emax = "$((${ProblemSizeArray[$ell]} * ${ProblemSizeArray[$ell]} * 1000))", model = ET5"

          ./RankingEDAsCEC -i ./PFSP_instance/tai"${Problem[$ell]}".txt\
                            -o ./PFSP_result/ET5/"${ProblemSizeArray[$ell]}"_"$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))"_ET5_"$times".txt\
                            -s "$times"\
                            -t PFSP\
                            -m ET -d C -v 0\
          ...
          echo "problem = tai"${Problem[$ell]}".txt, pop_size = "$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))", times = ${times}, Emax = "$((${ProblemSizeArray[$ell]} * ${ProblemSizeArray[$ell]} * 1000))", model = GMC"

          ./RankingEDAsCEC -i ./PFSP_instance/tai"${Problem[$ell]}".txt\
                            -o ./PFSP_result/GMC/"${ProblemSizeArray[$ell]}"_"$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))"_GMC_"$times".txt\
                            -s "$times"\
                            -t PFSP\
                            -m GM -d C -v 0\
                            -p "$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))"\
