...
pop_size_lst = [60, 120, 240, 480, 960, 1920]
...
for i in range(len(problem_instance_lst)):
    for pop_size in pop_size_lst:
            ...
            for t in range(repeat_time):
                ...
                f = open(f'./{problem_instance_lst[i]}_{pop_size}_MR_{t}.txt', 'r')
                s = f.read().splitlines()
            ...
            # print result data
            print(f'problem: {problem_instance_lst[i]}, population size: {pop_size}, model = MR:')
