...
# ell_lst = [24, 48, 76]
problem_instance_lst = ["gr24", "gr48"]
ell_lst = [24, 48]
...
for i in range(len(problem_instance_lst)):
    for j in range(len(pop_size_lst)):
            ...
            for t in range(repeat_time):
                ...
                f = open(f'./{problem_instance_lst[i]}_{pop_size_lst[j] * ell_lst[i]}_MST_{t}.txt', 'r')
                s = f.read().splitlines()
            ...
            # print result data
            print(f'problem: {problem_instance_lst[i]}, population size: {pop_size_lst[j] * ell_lst[i]}, model = MST:')
