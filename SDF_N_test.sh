...
do
  ...
  do
      ...
      do
          echo "ell = ${ProblemSizeArray[$ell]}, pop_size = "$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))", times = ${times}, Emax = "$((${ProblemSizeArray[$ell]} * ${ProblemSizeArray[$ell]} * 1000))", model = NO"

          ./RankingEDAsCEC -i ./SDF_instance/"${ProblemSizeArray[$ell]}".txt\
                            -o ./SDF_result/NO/"${ProblemSizeArray[$ell]}"_"$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))"_NO_"$times".txt\
                            -s "$times"\
                            -t SDF\
                            -m NO -d C -v 0\
          ...
          echo "ell = ${ProblemSizeArray[$ell]}, pop_size = "$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))", times = ${times}, Emax = "$((${ProblemSizeArray[$ell]} * ${ProblemSizeArray[$ell]} * 1000))", model = NT5"

          ./RankingEDAsCEC -i ./SDF_instance/"${ProblemSizeArray[$ell]}".txt\
                            -o ./SDF_result/NT5/"${ProblemSizeArray[$ell]}"_"$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))"_NT5_"$times".txt\
                            -s "$times"\
                            -t SDF\
                            -m NT -d C -v 0\
                            -p "$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))"\
