...
do
  ...
  do
      ...
      do
          ...
          echo "problem = tai"${Problem[$ell]}".txt, pop_size = "$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))", times = ${times}, Emax = "$((${ProblemSizeArray[$ell]} * ${ProblemSizeArray[$ell]} * 1000))", model = EO"

          ./RankingEDAsCEC -i ./PFSP_instance/tai"${Problem[$ell]}".txt\
                            -o ./PFSP_result/EO/"${ProblemSizeArray[$ell]}"_"$((${PopulationSizeArray[$pop]} * ${ProblemSizeArray[$ell]}))"_EO_"$times".txt\
                            -s "$times"\
                            -t PFSP\
                            -m EO -d C -v 0\
